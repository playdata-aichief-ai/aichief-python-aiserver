    ...
    def scan(self, img):
        ...
        if (scanned_img.shape[1] < 600) or (scanned_img.shape[0] < 900):
            ...
            if (scanned_img.shape[1] < 600) or (scanned_img.shape[0] < 900):
                ...
                img_FFT = np.fft.fft2(imgLog.copy(), (M, N))
                # low frequency 성분
                img_LF = np.real(np.fft.ifft2(
                    img_FFT.copy() * LPF_shift, (M, N)))
                # high frequency 성분
                img_HF = np.real(np.fft.ifft2(
                    img_FFT.copy() * HPF_shift, (M, N)))
                ...
                background = np.zeros(
                    (edged.shape[0], edged.shape[1], 3), dtype="uint8")
                ...
                for cnt in contours:
                    ...
                    if len(approx) >= 3:
                        approx2 = approx
                        cv2.drawContours(
                            background, [approx2], 0, (255, 255, 255), 1, lineType=cv2.LINE_AA)

                edged2 = cv2.Canny(background, 50, 300)

                contours, _ = cv2.findContours(
                    edged2, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
                contours = sorted(contours, key=cv2.contourArea, reverse=True)
                ...
                for cnt in contours:
                    ...
                    if len(approx) == 4:
                        cv2.drawContours(
                            background2, [approx], 0, (255, 0, 255), 1, lineType=cv2.LINE_AA)

                edged3 = cv2.Canny(background2, 50, 300)

                contours, _ = cv2.findContours(
                    edged3, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
                contours = sorted(contours, key=cv2.contourArea, reverse=True)
                ...
                if (scanned_img.shape[1] < 600) or (scanned_img.shape[0] < 900):
                    ...
                    scanned_img = four_point_transform(
                        img, doc_cnts.reshape(4, 2))
