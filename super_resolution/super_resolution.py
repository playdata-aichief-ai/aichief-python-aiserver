...
class Super_Resolution():
    ...
    def __init__(self):
        ...
        self.model = torch.load('./saved_model/model.pth', map_location=CFG['DEVICE']).eval().to(CFG['DEVICE'])
    ...
    def inference(self, img_dict):
        ...
        for key in img_dict:
            ...
            with torch.no_grad():
                ...
                img_lq = torch.from_numpy(img_lq).float().unsqueeze(0).to(CFG['DEVICE']) # add .to() if u have cuda 
                output = self.test(img_lq, window_size, SCALE)
                output = output.data.squeeze().float().cpu().clamp_(0, 1).numpy()
                if output.ndim == 3: output = np.transpose(output[[2, 1, 0], :, :], (1, 2, 0))  # CHW-RGB to HCW-BGR
